...
Agent::Agent(World* world, vec2 pos, float fov, float speed) {
	...
	self->AddComponent<SpriteRenderer>(TRIANGLE, Color(.2f, .8f, 0.6f, 1.0f));
	...
	self->AddComponent<RigidBody2D>(KINETIC, vec2(cos(tfr->Rotation), sin(tfr->Rotation)) * speed);
}
...
void Boid::update(float delta_time) {
	...
	for (uint32_t i = 0; i < n_agents; i++) {
		...
		float rr = (Math::random(i) - 0.5f) * agents[i].fov;
